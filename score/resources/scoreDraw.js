...
function ScoreLine(sheetElement, x, y, length) {
    ...
    //Step to create the score lines
    var lines = document.createElementNS(xmlns, "path");
    lines.setAttribute("d","M 0, 0 " + length + ", 0 M 0, 15 " + length + ", 15 M 0, 30 " + length + ", 30 M 0, 45 " + length + ", 45 M 0, 60 " + length + ", 60 M " + length + ", 0 " + length + " 60");
    lines.setAttribute("stroke", "#000");

    scoreLine.appendChild(lines);   //append the lines in the created scoreLine
    
    this.DrawScoreElement = function(scoreElement, x) {
        ...
        switch(scoreElement) {
            ...
            case ScoreElement.GClef:
                ...
                newScoreElement.setAttribute("transform","translate(" + (20 + x) + " 13)"); 
                break;
            ...
            case ScoreElement.TimeSig44:
                ...
                //translate the new element to its position
                newScoreElement.setAttribute("transform","translate(" + x + " 0)"); 
                break;
                
                //Element to draw simple bars in the score
            case ScoreElement.SimpleBar:
                newScoreElement = document.createElementNS(xmlns, "line");  //create new line
                newScoreElement.setAttribute("x1", x);
                newScoreElement.setAttribute("x2", x);
                newScoreElement.setAttribute("y2", 60);
                newScoreElement.setAttribute("stroke", "#000");
                break;           
        ...
        return newScoreElement; //return the just created element
    };
    
    this.DrawScoreNote = function(tempoDen, x, y) {
        var newScoreNote;
        ...
        switch(tempoDen) {
            case 1:
                ...
                newScoreNote.setAttribute("transform","translate(" + x + " " + y + ")"); 
                break;
...
var elem = DrawScoreLinesElement(ScoreElement.TimeSig44);
elem.style.cursor = "pointer";
elem.addEventListener("mouseover", function() {
    elem.filter = "drop-shadow( -0px -0px 0px #000 )";
...
/*var oi = new ScoreLine(document.documentElement,50.5,100.5,890);
oi.DrawScoreElement(ScoreElement.GClef, 10);
oi.DrawScoreElement(ScoreElement.TimeSig44, 60);
oi.DrawScoreElement(ScoreElement.SimpleBar, (890/2)*1);*/